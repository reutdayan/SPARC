...
def train(train_data, test_data=None):
    ...
    if FLAGS.save_embeddings:
        ...
        if FLAGS.model == "n2v":
            ...
            train_ids = tf.constant([[id_map[n]] for n in G.nodes() if not G.nodes[n]['val'] and not G.nodes[n]['test']],
                                    dtype=tf.int32)
            test_ids = tf.constant([[id_map[n]] for n in G.nodes() if G.nodes[n]['val'] or G.nodes[n]['test']],
                                   dtype=tf.int32)
            ...
            update_nodes = tf.scatter_nd(
                test_ids, update_nodes, tf.shape(model.context_embeds))
            no_update_nodes = tf.stop_gradient(tf.scatter_nd(
                train_ids, no_update_nodes, tf.shape(model.context_embeds)))
            model.context_embeds = update_nodes + no_update_nodes
            sess.run(model.context_embeds)

            # run random walks
            from graphsage.utils import run_random_walks
            nodes = [n for n in G.nodes() if G.nodes[n]["val"]
                     or G.nodes[n]["test"]]
            ...
            save_val_embeddings(
                sess, model, minibatch, FLAGS.validate_batch_size, log_dir(), mod="-test")
