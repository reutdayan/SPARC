...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx,
        ...
        if not n2v_retrain:
            ...
            self.val_edges = [e for e in G.edges() if G[e[0]]
                              [e[1]]['train_removed']]
            # print the number of train and val edges
        ...
        print(len([n for n in G.nodes() if not G.nodes[n]['test']
              and not G.nodes[n]['val']]), 'train nodes')
        print(len([n for n in G.nodes() if G.nodes[n]
              ['test'] or G.nodes[n]['val']]), 'test nodes')
        self.val_set_size = len(self.val_edges)

    def _n2v_prune(self, edges):
        def is_val(n): return self.G.nodes[n]["val"] or self.G.nodes[n]["test"]
    ...
    def _remove_isolated(self, edge_list):
        ...
        for n1, n2 in edge_list:
            if n1 not in self.G or n2 not in self.G:
                missing += 1
                continue
...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx,
        ...
        print("Node split: {:d} train, {:d} val, {:d} test ({:d} total)".format(
            len(self.train_nodes), len(self.val_nodes), len(self.test_nodes), len(G.nodes())))

        self._verify_isolation()

    def _verify_isolation(self):
        """Cold-start safety check tailored to GraphSAGESPARC.

        The SPARC variant intentionally fills *every* node's ``test_adj`` row
        with its top-K SPARC train neighbors, so test nodes are expected to
        have non-dummy entries in ``test_adj``. What MUST hold is that no test
        node leaks into the training adjacency ``adj``, which is built from
        edges with ``train_removed=False``. Edges incident to val/test nodes
        are flagged ``train_removed=True`` upstream, so train-adj rows for
        test nodes should be all-dummy.
        """
        dummy_idx = len(self.id2idx)
        adj_leaked = 0
        test_adj_with_neighbors = 0
        for n in self.test_nodes:
            idx = self.id2idx[n]
            if not np.all(self.adj[idx] == dummy_idx):
                adj_leaked += 1
            if not np.all(self.test_adj[idx] == dummy_idx):
                test_adj_with_neighbors += 1
        if adj_leaked > 0:
            print("WARNING: {:d} test node training-adj rows are NOT isolated "
                  "(cold-start leakage during training)!".format(adj_leaked))
        else:
            print("Verified: all {:d} test nodes are isolated in training-adj "
                  "(no cold-start leakage during training).".format(
                      len(self.test_nodes)))
        print("Note: {:d}/{:d} test nodes have non-dummy entries in test_adj "
              "(expected: each test node aggregates from its SPARC top-K "
              "train neighbors at eval time).".format(
                  test_adj_with_neighbors, len(self.test_nodes)))
